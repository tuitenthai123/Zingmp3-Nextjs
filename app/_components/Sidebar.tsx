      ...
      <nav className="flex-shrink-0">
        {menuItems.map((item, index) => (
          <Link href={item.route} key={item.label}>
            <div
              className={`group flex items-center justify-between gap-3 p-5 cursor-pointer ${
                selectedItem.type === "menu" && selectedItem.index === index
        ...
        <div className="pr-4">
          {canhan.map((item, index) => (
            <Link href={item.route} key={item.label}>
              <div
                className={`group flex items-center justify-between gap-3 p-5 cursor-pointer ${
                  selectedItem.type === "canhan" && selectedItem.index === index
                    ? "bg-gray-200 border-l-4 border-purple-600 text-purple-600 font-semibold"
                    : "text-gray-700 hover:text-purple-600 hover:bg-gray-200"
                }`}
